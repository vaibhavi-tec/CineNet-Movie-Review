...
from Adminsite.models import User,Movie, CastMember
...
def movies(request):
    ...
    return render(request,"user/movies.html",{'movie': movie})

def mainmovie(request, mid):
    movie = get_object_or_404(Movie, mid=mid)
    cast_members = movie.cast_members.all() 
    return render(request,"user/mainmovie.html",{'movie': movie, 'cast_members': cast_members})
...
def videoplay(request, mid):
    movie = get_object_or_404(Movie, mid=mid)
    return render(request,"user/videoplay.html",{'movie': movie})
# Create your views here.
